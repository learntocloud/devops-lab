...
# =============================================================================
validate_inc_001() {
    local DOCKERFILE="${AWS_DIR}/docker/Dockerfile"
    [ ! -f "$DOCKERFILE" ] && return

    if docker build -f "$DOCKERFILE" -t devops-lab-app-test "${AWS_DIR}" > /dev/null 2>&1; then
        local CID
...
# =============================================================================
validate_inc_002() {
    local COMPOSE="${AWS_DIR}/docker/docker-compose.yml"
    [ ! -f "$COMPOSE" ] && return
...
# =============================================================================
validate_inc_003() {
    local CI="${AWS_DIR}/github-actions/ci.yml"
    [ ! -f "$CI" ] && return
...
# =============================================================================
validate_inc_004() {
    local TF="${AWS_DIR}/terraform"
    [ ! -f "${TF}/main.tf" ] && return

    if grep -q "aws_vcp" "${TF}/main.tf" 2>/dev/null; then return; fi
    if grep -q "AmazonEKSCluserPolicy" "${TF}/main.tf" 2>/dev/null; then return; fi
    if grep -q 'service_ipv4_cidr.*=.*"10\.0\.' "${TF}/main.tf" 2>/dev/null; then return; fi
...
# =============================================================================
validate_inc_005() {
    local CD="${AWS_DIR}/github-actions/cd.yml"
    [ ! -f "$CD" ] && return
...
validate_inc_006() {
    ...
    if grep -q "ECR_REGISTRY" "${K}/app-deployment.yaml" 2>/dev/null; then return; fi
...
# =============================================================================
validate_inc_007() {
    local ALERTS="${AWS_DIR}/monitoring/alerts.json"
    [ ! -f "$ALERTS" ] && return

    local ENABLED SEVERITY PERIOD
    ENABLED=$(python3 -c "
import json
...
for alarm in data.get('alarms', []):
    if alarm.get('AlarmName') == 'pod-restart-alarm':
        print(alarm.get('Period'))
" 2>/dev/null || echo "")

    if [ "$ENABLED" = "True" ] && [ "$SEVERITY" = "2" ] && [ "$PERIOD" = "60" ]; then
        INC_007="resolved"
    fi
}
...
generate_verification_token() {
    ...
    local PAYLOAD='{"github_username":"'"$GITHUB_USER"'","date":"'"$COMPLETION_DATE"'","time":"'"$COMPLETION_TIME"'","timestamp":'"$TIMESTAMP"',"challenge":"devops-lab-aws","challenges":7,"instance_id":"'"$DEPLOYMENT_ID"'"}'
